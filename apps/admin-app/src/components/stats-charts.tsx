        ...
        {!loading && data.length > 0 ? (
          <div className="flex gap-4 text-right text-sm">
            <div>
              ...
              <p className="font-semibold tabular-nums text-slate-900">{totalOrders}</p>
            </div>
            <div>
        ...
        <div className="mt-6 flex h-56 items-end gap-2">
          {Array.from({ length: 14 }).map((_, index) => (
            <div key={index} className="flex flex-1 flex-col items-center gap-2">
              <div className="w-full animate-pulse rounded-t-lg bg-slate-100" style={{ height: `${20 + (index % 5) * 12}%` }} />
              <div className="h-2 w-6 animate-pulse rounded bg-slate-100" />
            </div>
          ))}
        </div>
      ) : data.length === 0 ? (
        <p className="mt-8 text-center text-sm text-slate-500">No order data yet.</p>
      ) : (
            ...
            return (
              <div key={point.date} className="group flex flex-1 flex-col items-center gap-2">
                <div className="relative flex h-full w-full items-end">
                  <div
                    className="w-full rounded-t-md bg-gradient-to-t from-orange-500 to-orange-400 transition-opacity group-hover:opacity-90"
                    style={{
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {loading ? (
        ...
        <p className="mt-6 text-sm text-slate-500">No product sales yet.</p>
      ) : (
        <div className="mt-4 space-y-3">
          {visibleProducts.map((product, index) => (
            <div key={product.product_id} className="rounded-xl bg-slate-50 px-4 py-3">
